...
| **Backend Language**     | Python                      | 3.13.8     | Primary language for the API and background worker.                  | A mature, robust language with a strong ecosystem for web development and data science.      |
...
-   **Strict Separation of Concerns:** Code **must** be strictly organized into the three defined layers:
    1.  **Endpoints (`/routers`):** This layer is responsible _only_ for handling HTTP requests and responses. It defines API routes, handles request validation using Pydantic models, and calls the appropriate service layer function. No business logic is permitted in this layer.
