...
* **Acceptance Criteria:**
  ...
  4. The system provides clear, user-centric, row-specific error feedback for any invalid data (e.g., "*Row 15: Duplicate School ID - A user with this School ID already exists for this university. Each user must have a unique ID.*").
...
* **Acceptance Criteria:**
  ...
  2. Upon this state change, an asynchronous, idempotent job is enqueued to update all associated `evaluation_submissions` to an 'invalidated\_pending\_cancellation' state and to notify affected users.
...
* **Acceptance Criteria:**
  ...
  7. The "Submit" button remains disabled until the countdown timer reaches zero and all *required* questions meet their respective validation rules (including minimum word count).
...
* **Acceptance Criteria:**
  ...
  2. A submission is flagged with a reason of "Low-Confidence" if it meets **both** of the following criteria: (A) the variance of all Likert-scale scores is zero, **AND** (B) the submission contains no meaningful qualitative feedback (i.e., all *optional* open-ended questions were left blank).
...
* **Acceptance Criteria:**
  ...
  6. *Dev Note:* All actions on a flagged evaluation must gracefully handle concurrent requests from multiple admins.
...
* **Acceptance Criteria:**
  ...
  5. A small, permanent disclaimer must be visible on the page, with text like: *"This AI provides suggestions based on data patterns. Please use them as a starting point for reflection, not as a final judgment."*
